...
http.createServer(function(req, res) {
	res.render = function(template, options){
		var str = fs.readFileSync(template, 'utf8'); //同步获取jade模板文件数据
		var fn = jade.compile(str, { filename: template, pretty: true }); //获取jade模板编译处理函数
		var page = fn(options); //调用fn将jade模板转化成为html文件数据字符
...
function upload(res, req){
    ...
    var form = new formidable.IncomingForm(); //获取formidable类的对象
	form.parse(req, function(error, fields, files) { //应用parse方法解析post数据
